...
var x = d3.scaleLinear().rangeRound([0, width / 2]);
...
d3.select("#dropdownDrug").selectAll("option").data(drug_options).enter().append("option").attr("value", function(option) {
	return option;
...
d3.select("#dropdownGender").selectAll("option").data(gender_options).enter().append("option").attr("value", function(option) {
	return option;
}).property("selected", function(d) {
	return d === gender
}).text(function(option) {
...
function updateOutput(gender, weight, time, percent, ounces, num_drinks, drug) {
	...
	if (currentBAC < .02) {
		defs.append("filter").attr("id", "motionFilter").attr("width", "300%") //Increase the width of the filter region to remove blur "boundary"
    ...
    .attr("width", "300%").attr("x", "-100%")
    .append("feGaussianBlur").attr("class", "blurValues")
	...
	barImage.transition().style("filter", "url(#motionFilter)").delay(500).duration(12000);
};
...
function ensureFilled() {
	if (document.getElementById('dropdownGender')
    .value != null && document.getElementById('w_value')
    .value != null && document.getElementById('time')
    .value != null && document.getElementById('percent')
    .value != null && document.getElementById('ounces')
    .value != null && document.getElementById('num_drinks')
    .value != null) {
		return true
	} else {
		return false;
...
function updateGraph(currentBAC, time) {
  ...
  .attr("x", width / 2 - 4)
  .attr("dx", "0.71em")
  .attr("text-anchor", "end").text("Hour");
	// draw y-axis
	g.append("g")
  .call(d3.axisLeft(y))
  ...
  .transition().delay(300).duration(13000).attr("stroke-dashoffset", 0);
}
